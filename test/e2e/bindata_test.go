...
func TestBindata(t *testing.T) {
	// e2e tests run in the root directory
	err := filepath.Walk("test/e2e/manifests", func(path string, info os.FileInfo, err error) error {
		if info == nil || info.IsDir() {
			return nil
		}
		bindataPath := strings.TrimPrefix(path, "test/e2e/manifests/")
	...
	if err != nil {
		t.Errorf("Error processing test/e2e/manifests")
	}
}
